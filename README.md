...
#### 2) 내 카페 추가하기
카페 이름, 주소, 사업자 번호를 모두 입력하고 추가를 누르면 카페 리스트가 생성 됩니다.
주소는 API를 사용하여 직접 입력하는 것이 아닌, 검색을 통해 정보를 넣을 수 있습니다.

#### 3) 내 카페 단일 정보 조회
추가한 카페의 단일 정보들을 조회할 수 있다. (카페 이름, 카페 주소, 사업자 번호, 만료 기간)

#### 4) 내 카페 단일 정보 수정
카페 이름, 사업자 번호, 카페 주소를 수정할 수 있습니다.

#### 5) 내 카페 정보 삭제
멤버쉽 정보를 확인하라는 경고문을 확인으로 넘어가면 삭제할 수 있습니다.

> 카페 쿠폰(테블릿과 모바일)
#### 1) 쿠폰 등록과 추가
핸드폰 번호 입력창을 만들어서 아직 미가입인 고객이라면 가입과 함께 쿠폰을 등록할 수 있게 합니다.
테블릿 화면에서 핸드폰 번호를 입력 받으면 모바일 화면에서 적립할 쿠폰의 갯수를 최종적으로 넘깁니다.

#### 2) 쿠폰 사용
모바일 화면에서 손님의 정보를 소켓으로 받으면 점주의 버튼 조작에 따라 쿠폰을 사용할 수 있습니다.

#### 3) 쿠폰 메모 수정

#### 4) 고객 리스트 조회와 고객 메모 등록&수정
쿠폰을 적립하면 고객 리스트를 표로 조회할 수 있습니다. 그리고 해당 고객에 따라 메모를 추가하거나 수정 할 수 있습니다.
